...
function onDeviceReadyForAjaxjs(){
   ...
   /*     cordova.plugins.backgroundMode.enable();
        ...
        cordova.plugins.backgroundMode.on('activate', function() {
            common.showToast('activate kaydı başarılı!','short','bottom',0);
        });

        cordova.plugins.backgroundMode.on('enable', function() {
            common.showToast('enable deneme mesajıdır','short','bottom',0);
        });

        cordova.plugins.backgroundMode.on('disable', function() {
            alert('ad');
            common.showToast('disable deneme mesajıdır','short','bottom',0);
        });

        cordova.plugins.backgroundMode.on('deactivate ', function() {
            alert('asd');
            common.showToast('deactivate deneme mesajıdır','short','bottom',0);
        });

        cordova.plugins.backgroundMode.on('failure ', function() {
            alert('asssd');
            common.showToast('failure deneme mesajıdır','short','bottom',0);
        });
